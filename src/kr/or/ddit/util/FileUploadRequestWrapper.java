 ...
 * 멀티파트일 때만 사용되는 것들 멀티파트가 아닌 그냥 파일인 경우에는 그냥 파일업로드를 이용하여 업로드한다. 
 */
public class FileUploadRequestWrapper extends HttpServletRequestWrapper {

	private static Logger logger = Logger.getLogger(FileUploadRequestWrapper.class);	// 로거 생성

    public static final String UPLOAD_DIRECTORY = "d:/A_TeachingMaterial/4.MiddleProject/other/files";		// 업로드 경로 설정
            ...
            for (int i = 0 ; i < list.size() ; i++) {
                ...
                if (fileItem.isFormField()) { // 폼필드인 경우...
                    ...
                    if (values == null) { // 처음 만드는 경우...
                        ...
                        String[] tempValues = new String[values.length + 1];  //기존 값에 추가 하려면 배열을 새로 만들어  
                        System.arraycopy(values, 0, tempValues, 0, values.length);   //이미 있던 배열의 내용을 복붙해서 맨 뒤에 새로 추가하는 값을 넣는다.
    ...
    public String getParameter(String name) {
        if (this.isMultipart) {
            ...
            return super.getParameter(name);   //getter setter의 그 getter
    }
     ...
     */
    public void delete() {  //임시저장폴더의 저장된 파일들을 삭제한다. 
        if (this.isMultipart) {
